...
foreach ($q as $row) {
    ...
    $pData['name'] = $pData['fields']['FirstName']['value'].' '.$pData['fields']['MiddleInitial']['value'].' '.$pData['fields']['LastName']['value'];
    $pData['phone'] = $pData['fields']['Offices'][0]['OfficeNumber']['value'];
    $pData['email'] = $pData['fields']['Email']['value'];
    $pData['position'] = $pData['fields']['Title']['fields']['Name']['value'];

    $pData['image'] = $pData['fields']['Photo']['value']['src'];
    ...
    $education = array();
    foreach($html->find('.RichTextWithShowMore.RichTextShowMoreSubsection__text') as $item)
    {
        if(strpos($item->innertext, 'Education</h4>') !== false)
        {
    ...
    foreach($html->find('.RichTextWithShowMore.RichTextShowMoreSubsection__text') as $item)
    {
        if(strpos($item->innertext, 'Admissions</h4>') !== false)
        {
            foreach($item->find('.RichText') as $item)
            {
    ...
    foreach($html->find('.AreasOfFocus__items') as $item)
    {
        if(strpos($item->innertext, 'Services') !== false)
        {
    ...
    $values['description'] = $html->find('.RichText', 0)->plaintext;
    ...
    {
        $primaryAddress = $pData['fields']['Offices'][0]['Office']['fields']['Name']['value'];
    }
