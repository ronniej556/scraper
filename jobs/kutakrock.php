...
foreach ($q as $row) {
    ...
    $values['phone_numbers'] = json_encode(array($pData['phoneNumber']));

    $values['email'] = $pData['emailAddress'];

    $fullAddress = '';

    $education = array();
    foreach($html->find('.side-accordion') as $item)
    {
    ...
    foreach($html->find('.list-section.list-section--left-align') as $item)
    {
        if(strpos($item->innertext, 'Related Services') !== false)
        {
    ...
    $values['description'] = $html->find('.read-more__content.rich-text', 0)->plaintext;

    $pData['image'] = $base_url.$pData['photo']['src'];
    $photo = $pData['image'];
    $thumb = $pData['image'];
