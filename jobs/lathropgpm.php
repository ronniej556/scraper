...
foreach ($q as $row) {
    ...
    $education = array();
    foreach($html->find('.bioSection') as $item)
    {
    ...
    $values['description'] = $html->find('#bioMain', 0)->plaintext;

    $pData['image'] = $base_url.'/'.$html->find('#bioPic img', 0)->src;
    $photo = $pData['image'];
    $thumb = $pData['image'];
