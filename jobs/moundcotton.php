...
foreach ($q as $row) {
    ...
    $values['description'] = trim(preg_replace('/\s+/', ' ', $html->find('#summary', 0)->plaintext));

    $photo = $html->find('[property="og:image"]', 0)->content;
    $thumb = $html->find('[property="og:image"]', 0)->content;
