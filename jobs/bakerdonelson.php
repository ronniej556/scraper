...
foreach ($q as $row) {
    ...
    $education = array();
    foreach($html->find('.sharedsidesection') as $item)
    {
        if(strpos($item->innertext, '<h2>Education</h2>') !== false)
        {
    ...
    foreach($html->find('.sharedsidesection') as $item)
    {
        if(strpos($item->innertext, '<h2>Admissions</h2>') !== false)
        {
            foreach($item->find('li') as $item)
            {
    ...
    foreach($html->find('.sharedsidesection') as $item)
    {
        if(strpos($item->innertext, '<h2>Practices</h2>') !== false)
        {
    ...
    $values['description'] = $html->find('.sharedbody-intro', 0)->plaintext;
