...
foreach ($q as $row) {
    ...
    $education = array();
    foreach($html->find('#atty_details_education') as $item)
    {
        foreach($item->find('.atty_edu_school') as $item)
        {
    ...
    foreach($html->find('.atty_related_practice_list.details_list') as $item)
    {
    ...
    $values['description'] = $html->find('#atty_tab_content p', 0)->plaintext;
