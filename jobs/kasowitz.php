...
$firm_name = 'Kasowitz Benson Torres LLP';
...
foreach ($q as $row) {
    ...
    $education = array();
    foreach($html->find('#panel-education') as $item)
    {
    ...
    foreach($html->find('#panel-admissions') as $item)
    {
        if(strpos($item->innertext, 'panel_inner') !== false)
        {
            foreach($item->find('li') as $item)
            {
    ...
    $values['description'] = $html->find('.col-md-7 p', 0)->plaintext;
