...
foreach ($q as $row) {
    ...
    $education = array();
    $ul = $html->find('#additional-info ul', 0);
    ...
    if($html->find('#additional-info ul', 1))
    {
        $ul = $html->find('#additional-info ul', 1);
        foreach($ul->find('li') as $item)
        {
    ...
    foreach($html->find('.related-content-links__industries') as $item)
    {
    ...
    $pData['image'] = $base_url.$pData['render_410_url'];
