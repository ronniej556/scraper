...
$spider_name = 'wc';
$firm_name = 'Williams & Connolly LLP';
...
foreach ($q as $row) {
    ...
    foreach($html->find('a') as $link)
    {
        ...
        {
            $values['vCard'] = $base_url.str_replace('../', '/', $link->href);
            break;
        }
    }
    ...
    $values['phone_numbers'] = json_encode(array($pData['email']));

    $values['email'] = $pData['phone'];

    $fullAddress = '';

    $education = array();
    foreach($html->find('#bio_details_education') as $item)
    {
    ...
    foreach($html->find('#bio_details_admissions') as $item)
    {
    ...
    foreach($html->find('.service-accordion__group') as $item)
    {
    ...
    $values['description'] = $html->find('.bio_short_overview', 0)->plaintext;
