...
foreach ($q as $row) {
    ...
    $education = array();
    foreach($html->find('.module.mod_education') as $item)
    {
    ...
    foreach($html->find('.module.mod_services') as $item)
    {
    ...
    $pData['image'] = $base_url.get_string_between($html, 'style="background: url(', ') no-repeat top left;');
