...
foreach ($q as $row) {
    ...
    $education = array();
    foreach($html->find('.col_list.edu_list') as $item)
    {
    ...
    foreach($html->find('.col_list.admission_list') as $item)
    {
        foreach($item->find('li') as $item)
        {
    ...
    foreach($html->find('#bio-details-pi') as $item)
    {
        foreach($item->find('li') as $item)
        {
    ...
    $values['description'] = $html->find('.intro', 0)->plaintext;
