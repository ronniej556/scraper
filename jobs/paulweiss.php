...
$firm_name = 'Paul, Weiss, Rifkind, Wharton & Garrison LLP';
...
foreach ($q as $row) {
    ...
    foreach($html->find('.education') as $item)
    {
        if(strpos(strtolower($item->innertext), ' ') !== false)
        {
            ...
            {
                $education[] = trim(preg_replace('/\s+/', ' ', $item->innertext));
            }
        }
    }
    ...
    foreach($html->find('.pratices-menu.bio-pratices-menu') as $item)
    {
        if(strpos(strtolower($item->innertext), '') !== false)
        {
    ...
    $positions = json_encode(array(str_replace(',  ', '', $html->find('.partner b', 0)->plaintext)));

    foreach($html->find('p') as $item)
    {
        if(strlen($item->plaintext)>150)
        {
            $values['description'] = $item->plaintext;
        }
    }
