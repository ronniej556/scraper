...
foreach ($q as $row) {
    ...
    $pData['url'] = str_replace(' ', '', $pData['url']);
    ...
    $education = array();
    foreach($html->find('.span-3.people-aside .block') as $item)
    {
        if(strpos($item->innertext, 'Education') !== false)
        {
            foreach($item->find('span[style="font-weight:bold"]') as $item)
            {
    ...
    foreach($html->find('.acc-details') as $item)
    {
    ...
    $values['description'] = trim(str_replace('Biography', '', $html->find('.span-6', 0)->plaintext));
