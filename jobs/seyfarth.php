...
foreach ($q as $row) {
    ...
    $pData['image'] = $html->find('img[height="770"]', 0)->src;

    $pData['image'] = 'https://sotodata.com/thumb.php?url='.$pData['image'];
    ...
    $education = array();
    foreach($html->find('.rc-collapse-item') as $item)
    {
    ...
    foreach($html->find('.grid__default.grid__xs-6.grid__md-12.grid__lg-3 div') as $item)
    {
