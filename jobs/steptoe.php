...
foreach ($q as $row) {
    ...
    $education = array();
    foreach($html->find('#education') as $item)
    {
    ...
    foreach($html->find('.bio-header__practices') as $item)
    {
        if(strpos(strtolower($item->innertext), '') !== false)
        {
    ...
    $values['description'] = nl2br($html->find('.rte', 0)->innertext);

    $pData['image'] = $base_url.$pData['professional_bio_square_url'];
    $photo = $pData['image'];
    $thumb = $pData['image'];
    ...
    if(!empty($values['vCard']))
    {
        ...
        if(filesize($spider_name.'_temp.vcf')<5242880)
        ...
        {
            $primaryAddress = $html->find('.bio-header__office p a', 0)->plaintext;
        }
