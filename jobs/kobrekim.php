...
foreach ($q as $row) {
    ...
    $education = array();
    foreach($html->find('.lawyer-creds-education ul') as $item)
    {
    ...
    $languages = array();
    foreach($html->find('.lawyer-creds-language ul') as $item)
    {
        foreach($item->find('li') as $item)
        {
    ...
    $values['description'] = $html->find('#biography', 0)->plaintext;
