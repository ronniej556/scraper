...
foreach ($q as $row) {
    ...
    if($html->find('.bio-vcard a', 0))
    {
        $values['vCard'] = $base_url.$html->find('.bio-vcard a', 0)->href;

        if(empty($linkedIn)) { $linkedIn = ''; }

        $values['names'] = json_encode(explode(' ', $pData['name']));

        $values['phone_numbers'] = json_encode(array($html->find('.bio-mobile', 0)->plaintext));
        ...
        $positions = json_encode(array($html->find('.attorney-title', 0)->plaintext));

        $values['description'] = $html->find('#inner-about', 0)->plaintext;

        $photo = $base_url.$html->find('.attorney-image img', 0)->src;
        $thumb = $base_url.$html->find('.attorney-image img', 0)->src;
