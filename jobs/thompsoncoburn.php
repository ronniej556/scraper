...
foreach ($q as $row) {
    ...
    $values['phone_numbers'] = json_encode(array(trim($pData['phone'])));
    ...
    $education = array();
    foreach($html->find('.acc-items') as $item)
    {
        if(strpos(strtolower($item->innertext), '') !== false)
        {
            foreach($item->find('li p') as $item)
            {
    ...
    foreach($html->find('#bar-court-admissions') as $item)
    {
        if(strpos(strtolower($item->innertext), ' ') !== false)
        {
    ...
    foreach($html->find('.bio-services') as $item)
    {
        if(strpos(strtolower($item->innertext), '') !== false)
        {
    ...
    $values['description'] = nl2br($html->find('.tabs-accordion-content-container div', 0)->innertext);

    $pData['image'] = $base_url.$html->find('.one-third-bio img', 0)->src;
