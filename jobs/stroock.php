...
$spider_name = 'stroock';
$firm_name = 'Stroock & Stroock & Lavan LLP';
...
foreach ($q as $row) {
    ...
    $education = array();
    foreach($html->find('#education li, #education p') as $item)
    {
    ...
    foreach($html->find('#admitted-to-practice ul li, #admitted-to-practice p') as $item)
    {
    ...
    foreach($html->find('#services') as $item)
    {
    ...
    $values['description'] = $html->find('#overview p', 0)->plaintext;

    if(empty($pData['image']))
    {
        $pData['image'] = $root.'/img/vecteezy_crossed-image-icon-picture-not-available-delete-picture_5720408.jpg';
    }
