...
foreach ($q as $row) {
    ...
    $education = array();
    foreach($html->find('#bio-expander-body__edu') as $item)
    {
        if(strpos($item->innertext, ' ') !== false)
        {
            foreach($item->find('div') as $item)
            {
    ...
    foreach($html->find('#bio-expander-body__area') as $item)
    {
        if(strpos($item->innertext, ' ') !== false)
        {
    ...
    $values['description'] = $html->find('#tabpanel-overview', 0)->plaintext;
