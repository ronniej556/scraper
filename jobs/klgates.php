...
foreach ($q as $row) {
    ...
    $education = array();
    foreach($html->find('.s-sidebar-expander.s-sidebar-expander--bio') as $item)
    {
    ...
    foreach($html->find('.sidebar-spacing') as $item)
    {
        if(strpos(strtolower($item->innertext), 'areas ') !== false)
        {
    ...
    $values['description'] = $html->find('.s-section-expander__body.s-wb-content', 0)->plaintext;
    ...
    {
        $primaryAddress = $html->find('.location-name', 0)->plaintext;
    }
