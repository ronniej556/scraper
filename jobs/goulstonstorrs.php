...
$firm_name = 'Goulston & Storrs Law Firm';
...
foreach ($q as $row) {
    ...
    $education = array();
    foreach($html->find('.education-list li') as $item)
    {
    ...
    foreach($html->find('.bar-admission') as $item)
    {
    ...
    foreach($html->find('#widget-2 ul li') as $item)
    {
    ...
    $values['description'] = trim(strip_tags($html->find('.description', 0)->innertext));
