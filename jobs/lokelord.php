...
$firm_name = 'Locke Lord LLP';

$q = $pdo->prepare('SELECT * FROM `queue` WHERE `status`=\'pending\' AND `spider_name`=? LIMIT 1000');
...
foreach ($q as $row) {
    ...
    $education = array();
    foreach($html->find('.people-detail__credential-group') as $item)
    {
    ...
    $values['description'] = $html->find('.detail-content__section-inner.rte', 0)->plaintext;
